/*
 COMPILE with:
    gcc -o L6_ofdm_freq L6_ofdm_freq_sync.c Transmitter.c Receiver.c Channel.c Auxiliary.c -lm -Wall

 Usage example:
    ./L6_ofdm_freq qpsk mmse 1 5000
...
int main(int argc, char** argv) {
    ...
    double freqOffset = 31.2;

    // Set parameters for m-sequence
    int seqLen = 127;           // the length of m sequence
    int syncSigStartIndex = 55; // first subcarrier to put m sequence
    ...
    for (int i = 0; i < sizeof(EbN0) / sizeof(double); i++) {
        ...
        for (int j = 0; j < numBlocks; j++) {
            ...
            // AWGN
            add_noise(rxSymI, rxSymQ, slotLen * (SYMBOLS_PER_BLOCK + CP_LEN));
            
            // Applying carrier freq offset to signal
            apply_carrier_freq_offset(freqOffset, SYMBOLS_PER_BLOCK, slotLen * (SYMBOLS_PER_BLOCK + CP_LEN), rxSymI, rxSymQ);
            ...
            // Estimate fractional freq offset
            double fractionalFreqOffset = carrier_freq_estimation_fractional(rxSymI, rxSymQ, slotLen * (SYMBOLS_PER_BLOCK + CP_LEN));
            // compensate fractional frequrncy offset
            apply_carrier_freq_offset(-fractionalFreqOffset, SYMBOLS_PER_BLOCK, slotLen * (SYMBOLS_PER_BLOCK + CP_LEN), rxSymI, rxSymQ);
            // Estimate integer freq offset
            int integerFreqOffset = carrier_freq_estimation_integer(rxSymI, rxSymQ, seqLen, syncSigStartIndex, Nid_2);
            // compensate integer frequrncy offset
            apply_carrier_freq_offset(-integerFreqOffset, SYMBOLS_PER_BLOCK, slotLen * (SYMBOLS_PER_BLOCK + CP_LEN), rxSymI, rxSymQ);
            ...
            for (int k = CP_LEN + CP_LEN + SYMBOLS_PER_BLOCK; k < SYMBOLS_PER_BLOCK + CP_LEN + CP_LEN + SYMBOLS_PER_BLOCK; k++) {
                rxSymI[k] /= (sqrtSNR[i]*SQRT_OF_2);
                rxSymQ[k] /= (sqrtSNR[i]*SQRT_OF_2);
            }

            // OFDM demodulation (CI is dropped)
            fft(rxSymI + 2 * CP_LEN + SYMBOLS_PER_BLOCK , rxSymQ + 2 * CP_LEN + SYMBOLS_PER_BLOCK , SYMBOLS_PER_BLOCK, rxEstI, rxEstQ);
    ...
    output_file_name("./results/", fileNameBER, "BER_FreqOffset", argv[1], argv[2], argv[3]);
