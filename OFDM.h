...
// Synchronization
void apply_carrier_freq_offset(float freq_offset, int freq_sampling, int lenIN, double* outFadedI, double* outFadedQ);

int timing_synch(double* inI, double* inQ, int lenIN);
int find_peak_index(double* inI, double* inQ, int lenIN);
void auto_corr(double* inI, double* inQ, int lenIN, double* outI, double* outQ, double* normalizedOutput); 
int find_max_index(double* maxValuBuffSSS, int len);
...
int save_signal(double* signal, int len, char* fileName);
